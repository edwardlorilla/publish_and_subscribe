...
class PublicationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->tags) {
            ...
            $users = $publication->user()->whereHas('tags', function ($query) use ($tags){
    ...
    public function update(Request $request, Publication $publication)
    {
        ...
        $publication->save();
			...
			$types = [];
			if($sync['attached']){
				$users = $publication->user()->whereHas('tags', function ($query) use ($attach){
            ...
            if($sync['detached']){
				$users = $publication->user()->whereHas('tags', function ($query) use ($detach){
